...
class RCOVTransformerLayer(MessagePassing):
    _alpha: OptTensor
...
class RCOVTransformer(nn.Module):
    def __init__(self, dim_features, dim_target, config, hidden_channels=None, linear=False,
        ...
        super(RCOVTransformer, self).__init__()
        ...
        self.graph_convs.append(
            RCOVTransformerLayer(in_channels=dim_features, out_channels=self.hidden_channels, edge_dim=edge_dim,
                                 bias=bias))
        self.lns.append(pyg.nn.norm.LayerNorm(hidden_channels, affine=False))
        for _ in range(1, self.num_layers):
            self.graph_convs.append(
                RCOVTransformerLayer(in_channels=self.hidden_channels, out_channels=self.hidden_channels,
                                     edge_dim=edge_dim,
                                     bias=bias))
